...
def main():
    ...
    m = pd.read_csv("clinical.csv")

    cols = list(set(m.columns) - set(["PTID","RID","subject", "ID","GROUP", "Group", "label", "Phase", "SITEID", "VISCODE", "VISCODE2", "USERDATE", "USERDATE2", "update_stamp", "DX_x","DX_y", "Unnamed: 0"]))
    X = m[cols].values 
    y = m["Group"].astype(int).values
    ...
    for seed in seeds:
        ...
        model = Sequential()
        model.add(Dense(128, input_shape = (179,), activation = "relu"))
